...
contract VIP181 is Context, ERC165, IVIP181, IVIP181Metadata {
        ...
        returns (uint256)
    {
        require(
            owner != address(0),
            "VIP181: balance query for the zero address"
        );
        ...
        returns (address)
    {
        ...
        require(
            owner != address(0),
            "VIP181: owner query for nonexistent token"
        );
        return owner;
    }
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "VIP181Metadata: URI query for nonexistent token"
        );
        ...
        returns (address)
    {
        require(
            _exists(tokenId),
            "VIP181: approved query for nonexistent token"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "VIP181: transfer caller is not owner nor approved"
        );
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        require(
            _checkOnVIP181Received(from, to, tokenId, _data),
        ...
        returns (bool)
    {
        require(
            _exists(tokenId),
            "VIP181: operator query for nonexistent token"
        );
        address owner = VIP181.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            VIP181.ownerOf(tokenId) == from,
            "VIP181: transfer of token that is not own"
        );
    ...
    function _checkOnVIP181Received(
        ...
        if (to.isContract()) {
            try
                IVIP181Receiver(to).onVIP181Received(
                    _msgSender(),
                    from,
                    tokenId,
                    _data
...
abstract contract VIP181URIStorage is VIP181 {
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "VIP181URIStorage: URI query for nonexistent token"
        );
        ...
        virtual
    {
        require(
            _exists(tokenId),
            "VIP181URIStorage: URI set of nonexistent token"
        );
...
contract PlanetNFTs is VIP181URIStorage, Ownable {
    ...
    modifier onlyWhenMintNotStarted() {
        require(
            _tokenIds.current() == 0,
            "Avalaible tokens not yet full initialised"
        );
        _;
    }
    ...
    function getPrice() public view returns (uint256) {
        uint256 tier_num = getTierNumber(msg.sender);
        ...
        return
            uint8(
                TIER_MINT_LIMIT[tier_num - 1] -
                    _mintCounter[tier_num - 1][pm_address]
            );
    }
    ...
    function setTokenURI(uint256 number, string memory tokenURI)
        external
        onlyOwner
    {
    ...
    function getRandomNumber(
        ...
        uint256 result = (randomHash % (top + 1));
        return result;
    }
    ...
    function startMintBatch(uint8 number) public payable {
        ...
        require(
            MAX_TOKEN >= number + _tokenPendingIds.current(),
        ...
        if (_giveawayAddress != msg.sender) {
            ...
            require(
                _mintCounter[tier_num - 1][msg.sender] + number <=
                    TIER_MINT_LIMIT[tier_num - 1],
                "Overflow maximum mint limitation"
            );

            address payable tgt = payable(msg.sender);
            (bool success1, ) = tgt.call{
                value: msg.value - getPrice() * number
            }("");
            require(success1, "Failed to refund");

            _mintCounter[tier_num - 1][msg.sender] =
                _mintCounter[tier_num - 1][msg.sender] +
                number;
        } else {
    ...
    function completeMintBatch(uint256 randomSeed) external {
        ...
        while (_firstPendingMintIndex < _lastPendingMintIndex) {
            ...
            _setTokenURI(
                tokenNumber,
                createFullUriTokenString(_baseUri, tokenNumber)
            );
